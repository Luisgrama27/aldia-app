...
function Splash({ onDone }) {
  ...
  return (
    <div style={{position:'fixed',inset:0,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',background:'var(--green)',fontFamily:'-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif',gap:24}}>
...
export default function App() {
  ...
  useEffect(() => {
    if (!usuario) { setCargandoProductos(false); return; }
  ...
  // ✅ FIX: esperar a que carguen los productos antes de decidir qué EmptyState mostrar
  const esUsuarioNuevo = !cargandoProductos && products.length === 0;
  ...
  return (
    <div style={S.screen}>
      ...
      {tab==='home'&&<>
        {cargandoProductos?(
          <div style={{display:'flex',alignItems:'center',justifyContent:'center',padding:48,color:'var(--text2)',fontSize:13}}>
            Cargando productos...
          </div>
        ):activos.length===0?(
          esUsuarioNuevo?(
